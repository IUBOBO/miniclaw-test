...
dir_img = r'E:\Xian-Jia He\GIS\coding\FLnet\data\Jiangyou\train\img/'
dir_boundary = r'E:\Xian-Jia He\GIS\coding\FLnet\data\Jiangyou\train\boundary/'
dir_mask = r'E:\Xian-Jia He\GIS\coding\FLnet\data\Jiangyou\train\musk/'
dir_distance = r'E:\Xian-Jia He\GIS\coding\FLnet\data\Jiangyou\train\distance/'
dir_checkpoint = r'E:\Xian-Jia He\GIS\coding\FLnet\data\Jiangyou\ckpts\SEANet/'
save_cp = True
 ...
 #   args.pretrained_model_path = './XJ_new_model_xinzeng/15.pt'
   ...
   # net_parameters_id = {}
    ...
    optimizer1 = torch.optim.SGD(
        [{'params': net_parameters_id['final1-2.weight'], 'lr': args.lr1},
  ...
  #  scheduler1 = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer1, int(1e10), eta_min=1e-5)
    ...
    train_loader = DataLoader(
        train, batch_size=4, shuffle=True, num_workers=8, pin_memory=True
    )
    val_loader = DataLoader(
        val, batch_size=4, shuffle=False, num_workers=8, pin_memory=True
    )

    writer = SummaryWriter(comment=f"LR_{0.0001}_BS_{3}_SCALE_{1}")
    global_step = 0
    ...
    for epoch in range(200):
        ...
        with tqdm(
            ...
            for batch in train_loader:
                ...
                true_distance = true_distance.to(device=device, dtype=mask_type)

                optimizer.zero_grad()

                output1, output2, output3 = model(imgs)
                loss1 = criterion1(output1, true_mask)
                loss2 = criterion2(output2, true_boundary)
                loss3 = criterion3(output3, true_distance)
