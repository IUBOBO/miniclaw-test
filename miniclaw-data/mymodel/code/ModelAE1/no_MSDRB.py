...
class BasicConv(nn.Module):
    def __init__(
        ...
        self.conv = nn.Conv2d(
            in_, out_, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation, groups=groups,
            bias=bias, )
...
class GuidedMaskRefineModule(nn.Module):
    """
    ...
    def __init__(self, in_channels):
        super(GuidedMaskRefineModule, self).__init__()
        # 辅助特征引导注意力生成（原始注意力）
    ...
    def forward(self, mask_feat, aux_feat):
        # 1. 原始辅助特征注意力调制
        attention_map = self.aux_attention(aux_feat)
        ...
        spatial_att = self.spatial_attention(
            torch.mean(aux_feat, dim=1, keepdim=True))  # [B,1,H,W]
        aux_spatial = aux_feat * spatial_att

        # 通道注意力
        channel_att = self.channel_attention(aux_feat)  # [B,C,1,1]
        aux_channel = aux_feat * channel_att

        # 4. 注意力加权融合
        aux_att = (aux_spatial + aux_channel) / 2
        combined = enhanced_mask * aux_att + enhanced_mask  # 残差连接
...
class Tripmodel(nn.Module):
    def __init__(self, input_channels=3, num_classes=1):
        ...
        self.conv1 = ConvModule(input_channels, 32)
        self.conv2 = ConvModule(32, 64)
        self.conv3 = ConvModule(64, 128)
        self.conv4 = ConvModule(128, 256)
        self.conv5 = ConvModule(256, 512)
        ...
        self.mask_fuse = GuidedMaskRefineModule(32)
    ...
    def forward(self, x):
        ...
        # 边界引导掩码
        mask_feat = self.mask_fuse(m4, b4)
