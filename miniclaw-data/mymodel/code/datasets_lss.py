...
root = r'F:\Shao-Shuai Liao\lss_data\sichuan'
...
def rand_crop(img1, mask, contour, size):
    ...
    if h < c_h or w < c_w:
        ...
        # distance_label = distance[s_h:e_h, s_w:e_w]
        # print('%d %d %d %d'%(s_h, e_h, s_w, e_w))
        return crop_im1, mask_im2, contour_label  # , distance_label
...
def rand_flip(img1, mask, contour):
    ...
    if r < 0.25:
        return img1, mask, contour  # ,distence
    elif r < 0.5:
...
def read_RSimages(mode, read_list=False):
    ...
    distance_dir = os.path.join(root, mode, 'distance')
    ...
    for idx, it in enumerate(data_list):
        if (it[-4:] == '.tif') or (it[-4:] == '.png'):
            ...
            distance_path = os.path.join(distance_dir, it)
...
class RS(data.Dataset):
    def __init__(self, mode, random_crop=False, crop_nums=6, sliding_crop=False, crop_size=256, random_flip=True):
        ...
        data_A, mask, contour, distance = read_RSimages(mode, read_list=False)
        if sliding_crop:
            data_A, mask, contour, distance = sliding_crop(data_A, mask, contour, [self.crop_size, self.crop_size])
    ...
    def __getitem__(self, idx):
        ...
        # return F.to_tensor(data_A), torch.from_numpy(mask).long(), torch.from_numpy(boundary).long()
